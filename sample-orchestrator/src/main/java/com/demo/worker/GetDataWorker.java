...
import com.demo.workflow.GetDataWorkflowImpl;
...
public class GetDataWorker {
    ...
    public void createWorker() {
        log.info("Registering GetDataWorker..");
        var workerFactory = WorkerFactory.newInstance(workflowClient, defaultWorkerFactoryOptions);
        var worker = workerFactory.newWorker(AllFunction.GETDATA.name(), defaultWorkerOptions);

        var completionClient = workflowClient.newActivityCompletionClient();
        worker.registerWorkflowImplementationTypes(defaultWorkflowImplementationOptions, GetDataWorkflowImpl.class);
